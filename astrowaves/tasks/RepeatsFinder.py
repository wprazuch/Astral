...
class RepeatsFinder:
    def _do_overlap(self, shape1, shape2, threshold):
        """Checks whether the shapes overlap with each other
        ...
        -------
        bool
            Decision whether shapes overlap with each other
        """
    ...
    def _multidim_intersect(self, arr1, arr2):
        ...
        no_intersected = (
            intersected.view(arr1.dtype).reshape(-1, arr1.shape[1]).shape[0]
        )
    ...
    def _merge_repeats(self, repeats_list):
        """Merges lists of repeats, as each shape has its repeats list, which overlap with each other"""
        changed = True
    ...
    def run(self, threshold, abs_df, neighbor_df):
        ...
        for shape1_id in tqdm(ids):
            ...
            shape1_id_repeats = self.search_for_repeats_of(
                shape1_id, neighbor_df, abs_df, threshold
            )
        ...
        if repeats:
            ...
            single_additional = [
                key for key, value in repeats_uq.items() if len(value) < 2
            ]
            singles.extend(single_additional)
            repeats_uq = {
                key: value for key, value in repeats_uq.items() if len(value) >= 2
            }
    ...
    def search_for_repeats_of(self, shape_id, neighbors_df, absolute_df, threshold):
        neighbors = neighbors_df.loc[neighbors_df["shape_id_1"] == shape_id][
            "shape_id_2"
        ].values
        repeats = []
...
def find_repeats(input_path, output_path, intersect_threshold=0.8):
    abs_df = pd.read_hdf(os.path.join(input_path, "segmentation_absolute.h5")).astype(
        "int16"
    )
    abs_df = abs_df[["id", "x", "y"]]

    neighbors_df = pd.read_csv(os.path.join(input_path, "neighbors.csv")).astype(
        "int16"
    )
...
def main():
    ...
    logging.basicConfig(
        filename=os.path.join(input_path, "logging.log"), level=logging.DEBUG
    )
